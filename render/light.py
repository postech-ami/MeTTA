...
class EnvironmentLight(torch.nn.Module):
    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        ...
        if specular:  # we are True
            roughness = ks[..., 1:2] # y component
            metallic  = ks[..., 2:3] # z component
            spec_col  = (1.0 - metallic)*0.04 + kd * metallic
            diff_col  = kd * (1.0 - metallic)
        else:
            diff_col = kd  # TODO:  torch.Size([1, 512, 512, 3])

        # FIXME:
        # if self.FLAGS != None:
        #     save_image = self.FLAGS.mat_interval and (self.iter % self.FLAGS.mat_interval == 0)
        #     if save_image:
        #         mat_vis(diff_col.clone().detach(), self.FLAGS.out_dir, "diff", self.iter)
        #         mat_vis(spec_col.clone().detach(), self.FLAGS.out_dir, "spec", self.iter)
        ...
        shaded_col = diffuse * diff_col  # TODO:  torch.Size([1, 512, 512, 3])
        # shaded_col min, max, mean, std: 0.0, 0.139, 0.053, 0.0654
        ...
        if specular:
            ...
            # shaded_col min, max, mean, std: 0.19, 0.329, 0.218, 0.030

        if self.FLAGS != None:
            save_image = self.FLAGS.lgt_interval and (self.iter % self.FLAGS.lgt_interval == 0)
            if save_image:
                lgt_vis(diff_col.clone().detach(), self.FLAGS.out_dir, "diff", self.iter)
                lgt_vis(shaded_col.clone().detach(), self.FLAGS.out_dir, "shaded", self.iter)
                lgt_vis((shaded_col * (1.0 - ks[..., 0:1])).clone().detach(), self.FLAGS.out_dir, "return", self.iter)
...
def _load_env_hdr(fn, scale=1.0, FLAGS=None):
    ...
    l = EnvironmentLight(cubemap, FLAGS)
    l.build_mips()

    return l

def load_env(fn, scale=1.0, FLAGS=None):
    if os.path.splitext(fn)[1].lower() == ".hdr":
        return _load_env_hdr(fn, scale, FLAGS)
    else:
...
def create_trainable_env_rnd(base_res, scale=0.5, bias=0.25, FLAGS=None):
    ...
    # base shape: torch.Size([6, 512, 512, 3])
    return EnvironmentLight(base, FLAGS)
      

######################################################################################
# Visualize diff_col and shaded_col
######################################################################################
def lgt_vis(x, out_dir, name, iter):
    ...
    x = x[0].cpu().numpy()  # we use batch_size, but only visualize first image of batch
    ...
    # prev_run_dirs = glob(os.path.join(out_dir, "lgt", name, "*"))
    # prev_run_ids = [int(p.split("/")[-1].split(".")[0]) for p in prev_run_dirs]
    # cur_run_id = max(prev_run_ids, default=-1) + 1

    x.save(os.path.join(out_dir, "lgt", name, f"{iter:06d}.png"))

def mat_vis(x, out_dir, name, iter):
    import matplotlib.pyplot as plt
    import numpy as np
    import torch
    from PIL import Image
    ...
    x.save(os.path.join(out_dir, "mat", f"{name}_{iter:06d}.png"))