...
def shade(
    ...
    if 'kd_ks_normal' in material:
        ...
        all_tex_jitter = material['kd_ks_normal'].sample(gb_pos + torch.normal(mean=0, std=0.01, size=gb_pos.shape, device="cuda"), args=None)  #  material['kd_ks_normal']: MLPTexture3D
        all_tex = material['kd_ks_normal'].sample(gb_pos, args=args)
    ...
    if 'no_perturbed_nrm' in material and material['no_perturbed_nrm']:
        perturbed_nrm = None
    # here, perturbed_nrm is not None
...
def render_layer(
    ...
    # rast_out_s: n, h, w, c (c is 4)
    gb_pos, _ = interpolate(mesh.v_pos[None, ...], rast_out_s, mesh.t_pos_idx.int())  # v_pos: vertex positions, t_pos_idx: mesh triangle index
    ...
    # Shade
    ################################################################################
    # gb_pos, gb_geometric_normal, gb_normal., gb_tangent: torch.Size([1, 512, 512, 3]), 
    # gb_mask: torch.Size([1, 512, 512, 1])
    # gb_texc: torch.Size([1, 512, 512, 2])
    # gb_texc_deriv.shape: torch.Size([1, 512, 512, 4])
    buffers = shade(gb_pos, gb_mask, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, view_pos, lgt, mesh.material, bsdf, args=args)
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf, args=args), rast)]
