...
def _translate(radius, device=None):
    # radius = torch.tensor([x, y, z], dtype=torch.float32, requires_grad=True)
...
def rotate_x(a, device=None):  # left-hand coordinate
...
def rotate_y(a, device=None):  # right-hand corodinate
    s, c = np.sin(a), np.cos(a)
...
def _rotate_x(a, device=None):  # left-hand coordinate
...
def _rotate_y(a, device=None):  # right-hand corodinate
    s, c = torch.sin(a), torch.cos(a)
    output = torch.eye(4, dtype=torch.float32)
    output[0][0] = c
    output[2][2] = c
